...
export default async (request) => {
  ...
  try {
    ...
    const sheets = getSheets(); const read = await sheets.spreadsheets.values.get({ spreadsheetId, range: `Respuestas de formulario 1!A${rowNumber}:L${rowNumber}` }); const row = read.data.values?.[0];
    ...
    const values = [[row[0], row[1], name, phone, document, body.member || "", body.origin || "", comment, body.type === "Sin clasificar" ? "" : body.type || "", row[9] || "", body.locality || "", body.birthDate || ""]];
    await sheets.spreadsheets.values.update({ spreadsheetId, range: `Respuestas de formulario 1!A${rowNumber}:L${rowNumber}`, valueInputOption: "USER_ENTERED", requestBody: { values } }); return reply(200, { ok: true });
