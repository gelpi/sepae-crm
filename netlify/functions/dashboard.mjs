...
export default async (request) => {
  ...
  try {
    ...
    const byDaySeller = {};
    for (const row of rows) { const date = dayKey(row[0]); if (!date) continue; const seller = row[1] || "Sin asignar"; const key = `${date}|${seller}`; byDaySeller[key] = (byDaySeller[key] || 0) + 1; }
    ...
    return reply(200, { total: rows.length, complete, phone: rows.filter((row) => row[3]).length, unclassified: rows.filter((row) => !row[8]).length, sellers: rank(count(1, "Sin asignar")), origins: rank(count(6, "Sin origen")).slice(0, 6), types: rank(count(8, "Sin clasificar")).slice(0, 6), dailyLoads });
