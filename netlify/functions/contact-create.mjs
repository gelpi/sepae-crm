...
export default async (request) => {
  ...
  try {
    ...
    await sheets.spreadsheets.values.append({ spreadsheetId, range: "Respuestas de formulario 1!A:P", valueInputOption: "USER_ENTERED", insertDataOption: "INSERT_ROWS", requestBody: { values: [[new Date().toISOString(), seller, name, phone, document, body.member || "", body.origin || "", body.comment || "", body.type === "Sin clasificar" ? "" : body.type || "", "", body.locality || "", body.birthDate || "", String(body.address || "").trim(), String(body.email || "").trim(), body.nextContact || "", randomUUID()]] } });
